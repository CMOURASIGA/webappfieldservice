...
content = content.replace(/const VERSION = "[\d\.]+";/, 'const VERSION = "1.5.0";');

// Remove the old injected mock block to avoid duplication
const oldMockStart = content.indexOf('// MOCK AGENDA E EQUIPE');
if (oldMockStart !== -1) {
  const oldMockEnd = content.indexOf('},', oldMockStart);
  content = content.substring(0, oldMockStart) + content.substring(oldMockEnd);
}

// Prepare the new rich mock block
const mockInjection = `
...
content = content.replace(/this\.logAudit\("usr-5", "Sistema restaurado para dados de demonstração estendidos"\);\n  \},/, `this.logAudit("usr-5", "Sistema restaurado para dados de demonstração estendidos");\n${mockInjection}\n  },`);
