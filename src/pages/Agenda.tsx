    ...
    return (
      <div className="agenda-grid flex h-[calc(100vh-250px)] flex-col">
        ...
        <div className="flex border-b-2 border-slate-300 bg-slate-100">
          ...
          {weekDays.map(day => (
            <div key={day.toISOString()} className={`flex-1 border-r border-slate-300 py-2 text-center last:border-r-0 ${isToday(day) ? 'bg-brand-100' : ''}`}>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="relative">
            {hours.map(hour => (
              <div key={hour} className="flex min-h-[60px] border-b border-slate-300 last:border-b-0">
                <div className="w-16 flex-shrink-0 border-r border-slate-300 bg-slate-100 py-2 text-center text-xs font-semibold text-slate-600">
                  {hour.toString().padStart(2, '0')}:00
                </div>
                {weekDays.map(day => (
                  <div 
                    key={`${day.toISOString()}-${hour}`} 
                    className={`relative flex-1 border-r border-slate-300 transition-colors last:border-r-0 ${isToday(day) ? 'bg-brand-50/50' : ''}`}
