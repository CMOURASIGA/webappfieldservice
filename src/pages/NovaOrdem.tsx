...
export const NovaOrdem = () => {
  ...
  const methods = useForm<OSFormData>({
    ...
    defaultValues: {
      ...
      // Os ativos são sempre escolhidos livremente na OS. Unidade e local
      // são apenas referências de atendimento e nunca preenchem este campo.
      assetIds: [],
  ...
  const onSubmit = async (data: OSFormData) => {
    ...
    try {
      ...
      storageService.logAudit(currentUser?.id || "sistema", "Criou Ordem de Serviço", newOrder.id, "WorkOrder", "", newOrder.status);
  ...
  const onInvalid = async (invalidFields: FieldErrors<OSFormData>) => {
    const targetTab = invalidFields.unitId || invalidFields.locationId ? "dados-gerais"
      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit, onInvalid)}>
          ...
          <div className="flex justify-end border-t border-slate-200 pt-5">
            <Button type="submit" disabled={isSubmitting} className="bg-blue-700 text-white hover:bg-blue-800 active:bg-blue-900">
