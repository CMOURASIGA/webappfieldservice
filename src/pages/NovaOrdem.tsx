      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit)}>
          <Card className="overflow-hidden border border-slate-200 shadow-sm">
            ...
            <CardFooter className="justify-end gap-3 border-t border-slate-200 px-6 py-4"><Button type="button" variant="secondary" onClick={() => navigate(-1)}>Cancelar</Button><Button type="submit" disabled={!isValid || isSubmitting} className="gap-2"><Save className="h-4 w-4" />{isSubmitting ? "Salvando..." : "Salvar Ordem de Serviço"}</Button></CardFooter>
          </Card>
        </form>
      </FormProvider>
    </div>
