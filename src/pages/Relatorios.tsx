...
export const Relatorios = () => {
  ...
  return <div className="space-y-6"><OperationalPageHeader title="Relatórios" description="Escolha o relatório, informe os parâmetros e gere uma versão pronta para imprimir ou salvar em PDF." backTo="/" />
    <section aria-label="Catálogo de relatórios" className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">{catalog.map((item) => {
      const isSelected = selected === item.id;
      return <button key={item.id} onClick={() => setSelected(item.id)} className="flex min-h-48 flex-col rounded-xl p-5 text-left transition hover:-translate-y-0.5 hover:shadow-md" style={{ border: `2px solid ${isSelected ? "#1246a0" : "#cbd5e1"}`, backgroundColor: isSelected ? "#eff6ff" : "#ffffff", boxShadow: isSelected ? "0 5px 14px rgba(18,70,160,.16)" : "0 2px 7px rgba(15,23,42,.08)" }}>
        <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-lg bg-brand-50 text-brand-700"><item.icon className="h-5 w-5" /></div>
        <p className="text-xs font-bold uppercase tracking-wide text-slate-500">{item.area}</p><h2 className="mt-1 font-bold text-slate-900">{item.title}</h2><p className="mt-2 text-sm leading-5 text-slate-600">{item.description}</p>
        {isSelected && <span className="mt-auto pt-3 text-xs font-bold uppercase tracking-wide text-brand-700">Relatório selecionado</span>}
      </button>;
    })}</section>
